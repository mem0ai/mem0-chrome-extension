...
function injectMem0Button() {
  ...
  if (window.OPENMEMORY_UI && OPENMEMORY_UI.mountOnEditorFocus) {
      ...
      if (!document.getElementById('mem0-icon-button')) {
        OPENMEMORY_UI.resolveCachedAnchor({ learnKey: location.host + ':' + location.pathname }, null, 24*60*60*1000)
          .then(function(hit){
            ...
            var cfg = (typeof SITE_CONFIG !== 'undefined' && SITE_CONFIG.gemini) ? SITE_CONFIG.gemini : null;
    ...
    OPENMEMORY_UI.mountOnEditorFocus({
      ...
      editorSelector: (typeof SITE_CONFIG !== 'undefined' && SITE_CONFIG.gemini && SITE_CONFIG.gemini.editorSelector) ? SITE_CONFIG.gemini.editorSelector : 'textarea, [contenteditable="true"], input[type="text"]',
      deriveAnchor: (typeof SITE_CONFIG !== 'undefined' && SITE_CONFIG.gemini && typeof SITE_CONFIG.gemini.deriveAnchor === 'function') ? SITE_CONFIG.gemini.deriveAnchor : function(editor){ return editor.closest('form') || editor.parentElement; },
      placement: (typeof SITE_CONFIG !== 'undefined' && SITE_CONFIG.gemini && SITE_CONFIG.gemini.placement) ? SITE_CONFIG.gemini.placement : { strategy: 'inline', where: 'beforeend', inlineAlign: 'end' },
