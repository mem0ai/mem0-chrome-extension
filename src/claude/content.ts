...
function addToConversationHistory(role: MessageRole, content: string): void {
  ...
  const message = { role, content: trimmedContent, timestamp: Date.now() };
...
function getConversationContext(
  ...
  return contextMessages.slice(-contextSize).map(msg => ({ role: msg.role, content: msg.content }));
}
...
function computeActive(): boolean {
  ...
  const val = el ? (el.textContent || (el as HTMLInputElement).value || '').trim() : '';
  return val.length > 3;
}
...
function findOrCreateButtonContainer(): HTMLElement | null {
  ...
  if (toolsTrigger) {
    const parent =
      toolsTrigger.closest('.relative.flex-1.flex.items-center.gap-2') ||
      toolsTrigger.closest('.relative.flex-1') ||
      toolsTrigger.parentElement;
    if (parent) {
...
function insertButtonIntoContainer(
  ...
  // Find the parent container that contains input-tools-menu-trigger
  const toolsTrigger = document.querySelector('#input-tools-menu-trigger');
  if (toolsTrigger) {
    // Find the parent container with flex shrink-0
    const flexShrinkDiv = toolsTrigger.closest(
      'div[class*="flex"][class*="shrink-0"]'
    ) as HTMLElement;
    if (flexShrinkDiv && flexShrinkDiv.parentElement) {
      // Insert the button into the parent container, but outside the flex shrink-0 div
      flexShrinkDiv.parentElement.insertBefore(mem0ButtonContainer, flexShrinkDiv.nextSibling);
    } else {
      // Fallback: insert next to toolsTrigger
      (toolsTrigger.parentElement as HTMLElement)?.insertBefore(
        mem0ButtonContainer,
        toolsTrigger.nextSibling
      );
    }
  } else {
    // If no toolsTrigger, find the send button
    const sendButton =
      buttonContainer.querySelector('button[aria-label="Send Message"]') ||
      buttonContainer.querySelector('button[aria-label="Send message"]');
...
function getInputValue(): string | null {
  ...
  if ((inputElement as HTMLElement).tagName.toLowerCase() === 'p') {
...
// Function to add the Mem0 button (Claude): similar to GPT
...
// Modified function to handle Mem0 button click - now it searches and adds immediately
async function handleMem0Click(): Promise<void> {
  ...
  try {
    ...
    // Wait for the orchestrator response
    const items = await searchSession.runSearchAndWait(claudeSearch, message);
...
function captureAndStoreMemorySnapshot(): void {
  // Get the message content (Claude)
  const inputElement =
    (document.querySelector('div[contenteditable="true"].ProseMirror') as HTMLDivElement) ||
    (document.querySelector('div[contenteditable="true"]') as HTMLDivElement) ||
    (document.querySelector('textarea') as HTMLTextAreaElement);
  ...
  chrome.storage.sync.get(
    ...
    function (items) {
      ...
      const messages = getConversationContext(false);
...
// Update the initialization function to add the Mem0 icon button
function initializeMem0Integration(): void {
  ...
  // SPA navigation handling (mirrors Claude)
