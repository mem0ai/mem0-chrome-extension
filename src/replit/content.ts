...
try {
  ...
  function addSendButtonListener(): void {
    ...
    if (textarea) {
      if (!textarea.dataset.mem0KeyListener) {
        ...
        textarea.addEventListener("keyup", function (_event) {
          lastInputValue =
            textarea.textContent || (textarea as ExtendedHTMLElement).innerText || "";
        });
      }
      ...
      const memoryItems: MemoryItem[] = (responseData as MemorySearchResponse).map(
        (item, index: number) => {
          return {
            id:
              item.id ||
              `memory-${Date.now()}-${index}-${Math.random().toString(36).substring(2, 9)}`,
            text: item.memory,
  ...
  function initializeMem0Integration(): void {
    ...
    mainObserver = new MutationObserver(async _mutations => {
